...
def demo_navigate_packages(project: Any) -> None:
    ...
    try:
        ...
        if packages and len(packages) > 0:
            print(f"✓ Found {len(packages)} top-level package(s)")
            ...
            for i, pkg in enumerate(packages, 1):
                ...
                # Try to get nested packages
                try:
                    nested_packages = pkg.getNestedPackages()
                    if nested_packages and len(nested_packages) > 0:
                        print(f"  - Nested packages: {len(nested_packages)}")
                        for nested_pkg in nested_packages:
                            print(f"    • {nested_pkg.getName()}")
                except Exception as e:
                    print(f"  - Nested packages: Unable to retrieve ({e})")
                ...
                try:
                    ...
                    print(f"  - Classes: Unable to retrieve ({e})")
        else:
    ...
    for meta_class in metaclasses:
        try:
            ...
            if elements and len(elements) > 0:
                print(f"✓ Found {len(elements)} {meta_class}(es)")

                # Show first few elements
                display_count = min(5, len(elements))
                for i, element in enumerate(elements[:display_count], 1):
                    print(f"  {i}. {element.getName()} (GUID: {element.getGUID()[:8]}...)")
...
def demo_find_specific_element(project: Any) -> None:
    ...
    # Try to find some common element names
    search_names = ["Main", "Application", "Controller", "Model", "View"]

    for name in search_names:
        found = False

        # Search for different metaclasses
        for meta_class in ["Class", "Actor", "Package", "UseCase"]:
            try:
                print(f"\nSearching for '{name}' (type: {meta_class})...")
                element = project.findNestedElement(name, meta_class)

                if element:
                    print(f"✓ Found: {element.getMetaClass()} '{element.getName()}'")
                    print(f"  - GUID: {element.getGUID()}")
                    print(f"  - Full name: {element.getFullName()}")
...
def demo_display_class_details(project: Any) -> None:
    ...
    try:
        ...
        for i, cls in enumerate(classes[:display_count], 1):
            ...
            print(f"  - Full name: {cls.getFullName()}")
            ...
            try:
                ...
                for attr in attributes[:3]:  # Show first 3
                    print(f"    • {attr.getName()}: {attr.getType()}")
                if len(attributes) > 3:
            ...
            try:
                ...
                for op in operations[:3]:  # Show first 3
                    print(f"    • {op.getName()}(): {op.getReturnResult()}")
                if len(operations) > 3:
            ...
            # Get relationships
            try:
                supers = cls.getSuperClasses()
                if supers and len(supers) > 0:
                    print(f"  - Superclasses: {len(supers)}")
                    for sup in supers:
                        print(f"    • {sup.getName()}")
            except Exception as e:
                print(f"  - Superclasses: Unable to retrieve ({e})")
...
def main() -> None:
    ...
    print("=" * 60)
    print("\nThis demo requires an open Rhapsody project with model elements.")
